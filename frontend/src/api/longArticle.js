...
export const longArticleApi = {
  ...
  listArticles: async (params = {}) => {
    const response = await axios.get(`${API_BASE_URL}/api/long-article/list`, { params });
    return response.data;
  },
  ...
  generateStream: (articleId) => {
    return new EventSource(`${API_BASE_URL}/api/long-article/generate/${articleId}`);
  },

  // 恢复生成
  resumeStream: (articleId) => {
    return new EventSource(`${API_BASE_URL}/api/long-article/resume/${articleId}`);
  }
};
