...
def list_events(project_id: int, db: Session = Depends(get_db)):
    ...
    events = (
        ...
        .order_by(Event.order_index, Event.created_at)
        .all()
    )
    return events
...
def get_event(project_id: int, event_id: int, db: Session = Depends(get_db)):
    """获取事件详情"""
    event = db.query(Event).filter(Event.id == event_id, Event.project_id == project_id).first()
    if not event:
...
def update_event(
    project_id: int, event_id: int, event_update: EventUpdate, db: Session = Depends(get_db)
):
    """更新事件"""
    event = db.query(Event).filter(Event.id == event_id, Event.project_id == project_id).first()
    if not event:
...
def reorder_event(project_id: int, event_id: int, new_index: int, db: Session = Depends(get_db)):
    """重新排序事件"""
    event = db.query(Event).filter(Event.id == event_id, Event.project_id == project_id).first()
    if not event:
        raise HTTPException(status_code=404, detail="Event not found")
