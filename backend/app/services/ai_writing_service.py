...
- 可以提出建设性的问题帮助用户思考"""
    ...
    def _build_messages(
        ...
        """构建 AI 对话消息"""
        messages = [{"role": "system", "content": AIWritingService.SYSTEM_PROMPT}]
        ...
        # 添加当前文档内容
        messages.append({"role": "user", "content": f"当前文档内容：\n{content}"})
    ...
    @staticmethod
    async def process_request(db: Session, request: AIRequest, document_content: str) -> str:
        """处理 AI 请求（非流式）"""
        # 获取记忆上下文
        memory_context = ""
    ...
    async def stream_request(
        db: Session, request: AIRequest, document_content: str
    ...
    @staticmethod
    async def chat(
        db: Session, document_id: int, messages: list[ChatMessage], include_memory: bool = True
    ) -> AsyncGenerator[str, None]:
        """自由对话模式"""
        from app.models.models import Document
