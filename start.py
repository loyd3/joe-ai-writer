...
def log(msg, color=Colors.GREEN):
    print(f"{color}[Joe AI Writer]{Colors.END} {msg}")
...
def main():
    parser = argparse.ArgumentParser(description="Joe AI Writer 启动脚本")
    ...
    try:
        ...
        # 打印访问信息
        print("\n" + "="*60)
        print(f"  {Colors.GREEN}Joe AI Writer 已启动!{Colors.END}")
        print("="*60)
        if not args.frontend_only:
